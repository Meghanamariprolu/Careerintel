            ...
            <div className="max-w-6xl mx-auto relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {mentors.map((mentor, idx) => (
                        ...
                        >
                            <div className={`absolute inset-0 bg-gradient-to-br from-white/10 to-transparent rounded-[3rem] blur-xl opacity-0 group-hover:opacity-100 transition duration-500`} />

                            <div className="relative h-full p-10 rounded-[3rem] bg-white/[0.03] border border-white/10 group-hover:border-white/20 transition-all flex flex-col backdrop-blur-3xl overflow-hidden">
                                <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity">
                                    <mentor.icon className="h-32 w-32" />
                                </div>

                                <div className="mb-8">
                                    <div className={`h-16 w-16 rounded-2xl ${mentor.bg} flex items-center justify-center border border-white/5 mb-6 group-hover:scale-110 transition-transform duration-500`}>
                                        <mentor.icon className={`h-8 w-8 ${mentor.color}`} />
                                    </div>
                                    <h3 className="text-base md:text-xl font-black text-white group-hover:text-indigo-400 transition-colors uppercase italic tracking-tighter">{mentor.name}</h3>
                                    <p className="text-[10px] md:text-xs font-black uppercase tracking-[0.2em] text-white/30 mt-1">{mentor.role}</p>
                                </div>
                                ...
                                <div className="flex-grow">
                                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/5 mb-6">
                                        ...
                                        <span className="text-[10px] md:text-xs font-bold text-white/60 tracking-tight">{mentor.specialization}</span>
                                    </div>
                                    <p className="text-xs md:text-base text-white/50 leading-relaxed font-medium mb-8">
                                        {mentor.description}
                                    </p>
                                </div>
                                ...
                                <div className="pt-8 border-t border-white/5">
                                    ...
                                    <p className="text-xs md:text-base font-black italic text-white/80 tracking-tight leading-snug">
                                        "{mentor.philosophy}"
                                    </p>
                                </div>
                            </div>
