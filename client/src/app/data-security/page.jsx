...
const measures = [
    {
        ...
        points: ["Passwords are hashed using bcrypt with a work factor of 12 — we never store plaintext passwords","Minimum password length of 8 characters is enforced","Passwords are never logged or included in error messages",
        ]
    },
    {
        icon: Lock,
        ...
        points: ["All data is transmitted over HTTPS with TLS 1.2+ encryption","HTTP Strict Transport Security (HSTS) is enforced","API responses include security headers (Helmet.js) to prevent common attacks like XSS and clickjacking",
        ]
    },
    {
        icon: Server,
        ...
        points: ["MongoDB Atlas with TLS-encrypted connections and IP allowlisting","Database credentials are stored securely as environment variables, never in code","Regular automated database backups with encryption at rest",
        ]
    },
    {
        icon: ShieldCheck,
        ...
        points: ["JWT tokens with 30-day expiration windows and server-side validation","Tokens are transmitted via secure Authorization headers, not exposed in URLs","Session tokens are invalidated upon logout",
        ]
    },
    {
        icon: Eye,
        ...
        points: ["Role-based access control (RBAC) ensures users can only access their own data","All API endpoints require valid authentication tokens","Administrative actions require elevated privilege verification",
        ]
    },
    {
        icon: AlertTriangle,
        ...
        points: ["Input validation using Zod on all API endpoints to prevent injection attacks","Rate limiting applied to sensitive endpoints like login and registration","Regular dependency audits using npm audit to identify and patch vulnerabilities",
        ]
    },
];
    ...
    return (
        <div className="min-h-screen bg-[#050510] text-white font-sans overflow-x-hidden">
            <div className="relative py-20 px-6 text-center overflow-hidden">
                ...
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 mb-6 text-xs   text-emerald-300">
                    <Lock className="h-3 w-3" /> Security Architecture
                </motion.div>
                <h1 className="text-3xl md:text-5xl font-light  text-white mb-4">Data Security</h1>
