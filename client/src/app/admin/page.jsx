...
export default function AdminDashboardPage() {
    ...
    if (errorMsg) {
        return <div className="text-destructive  p-6">Error: {errorMsg}</div>
    }

    return (
        <div className="space-y-8">
            <div>
                <h1 className="text-3xl  tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-neon-cyan to-deep-purple">Platform Analytics</h1>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Card className="border-white/10 bg-slate-900/40 backdrop-blur-md">
                    ...
                    <CardContent>
                        <div className="text-2xl  text-white">{metrics?.totalUsers}</div>
                    </CardContent>
                </Card>
                <Card className="border-white/10 bg-slate-900/40 backdrop-blur-md">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 text-indigo-300">
                        <CardTitle className="text-sm ">Total Roadmaps</CardTitle>
                        <FileText className="h-4 w-4" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl  text-white">{metrics?.totalRoadmaps}</div>
                    </CardContent>
                </Card>
                <Card className="border-white/10 bg-slate-900/40 backdrop-blur-md">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 text-neon-cyan">
                        <CardTitle className="text-sm ">Recent Activity (7d)</CardTitle>
                        <Activity className="h-4 w-4" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl  text-white">+{metrics?.recentRoadmaps} <span className="text-xs  text-slate-400">generated</span></div>
                    </CardContent>
                </Card>
                <Card className="border-white/10 bg-slate-900/40 backdrop-blur-md">
                    ...
                    <CardContent>
                        <div className="text-2xl  text-white">{Math.round(metrics?.avgResponseTime || 0)} <span className="text-xs  text-slate-400">ms</span></div>
                    </CardContent>
                </Card>
            </div>

            <div>
                <h2 className="text-xl  tracking-tight mb-4">Recent Users</h2>
                <Card>
                                    ...
                                    {usersList.slice(0, 10).map((u) => (
                                        <tr key={u.id || u._id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`px-2 py-1 rounded-full text-[10px]  uppercase tracking-wider ${u.role === 'admin' ? 'bg-deep-purple/20 text-deep-purple border border-deep-purple/30' : 'bg-neon-cyan/20 text-neon-cyan border border-neon-cyan/30'}`}>
                                                    {u.role}
                                                </span>
                                            </td>
