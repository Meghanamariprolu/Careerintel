    ...
    return (
        <div className="min-h-screen bg-[#050510] text-white font-sans overflow-x-hidden">
            <div className="relative py-20 px-6 text-center overflow-hidden">
                ...
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 mb-6 text-xs   text-blue-300">
                    <Shield className="h-3 w-3" /> Legal
                </motion.div>
                <h1 className="text-3xl md:text-5xl font-light  text-white mb-4">Privacy Policy</h1>
                <p className="text-white/60 text-sm">Last updated: March 7, 2025</p>
            </div>
                ...
                <div className="space-y-10">
                    {sections.map((section, idx) => (
                        <motion.section key={section.title} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}>
                            <h2 className="text-base font-light text-white mb-4  border-b border-white/5 pb-3">{section.title}</h2>
