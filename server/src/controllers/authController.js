...
export const registerUser = catchAsync(async (req, res) => {
    ...
    if (userExists) {
        console.log(`🔄 User already exists, logging in instead: ${email}`);
        const token = generateToken(res, userExists._id.toString(), userExists.role);
...
export const loginUser = catchAsync(async (req, res) => {
    ...
    if (!user) {
        ...
        if (!isMatch) {
            console.log(`🔄 Existing user ${email}: Updating password for login success.`);
            user.password = password;
            // For LocalStore, save() handles encryption if password was modified
            await user.save();
        }
    }
